...
import co.edu.frontend.model.Calificacion;
...
public class EstudianteCursoController { // Puedes usar este controlador o crear uno nuevo como EstudianteHistorialController
    ...
    private final String ESTUDIANTE_API_URL = "http://localhost:8081/estudiantes"; // Backend EstudianteController
    private final String ESTUDIANTE_CURSO_API_URL = "http://localhost:8081/estudiante-cursos"; // Backend EstudianteCursoController
    private final String HISTORIAL_API_URL = "http://localhost:8081/historial";
    ...
    @GetMapping("/actual")
    public String mostrarMatriculaActual(HttpSession session, Model model) {
        LoginResponse loginData = (LoginResponse) session.getAttribute("username");
        ...
        try {
            ...
            if (estudianteResponseEntity.getStatusCode() == HttpStatus.OK && estudianteResponseEntity.getBody() != null) {
                ...
                // PASO 2: Obtener la lista de EstudianteCurso usando el estudianteId
                String getHistorialUrl = ESTUDIANTE_CURSO_API_URL + "/estudiante-cursados/" + estudianteId;
                logger.info("Llamando a API para obtener historial de cursos: {}", getHistorialUrl);
                ...
                if (historialResponse.getStatusCode() == HttpStatus.OK && historialResponse.getBody() != null) {
                    ...
                    // PASO 3: Iterar para obtener y asignar las calificaciones y definitiva a cada EstudianteCurso
                    for (EstudianteCurso ec : historialCursos) {
                        if (ec.getId() != null) {

                            // Obtener las calificaciones parciales (P1, P2, P3, EX)
                            obtenerCalificacionesParciales(ec, requestEntity);

                            // Obtener la definitiva
                            obtenerDefinitiva(ec, requestEntity);
                        }
                    }
        ...
        return "estudiante_materia"; // Nombre de tu archivo JSP
    }

    /**
     * Metodo auxiliar para obtener las calificaciones parciales de un EstudianteCurso
     */
    private void obtenerCalificacionesParciales(EstudianteCurso ec, HttpEntity<Void> requestEntity) {
        String getCalificacionUrl = HISTORIAL_API_URL + "/estudiante-curso/" + ec.getId() + "/calificaciones";
        logger.info("Llamando a API para obtener calificaciones del EstudianteCurso ID {}: {}", ec.getId(), getCalificacionUrl);

        try {
            ParameterizedTypeReference<List<Calificacion>> responseType =
                    new ParameterizedTypeReference<List<Calificacion>>() {};

            ResponseEntity<List<Calificacion>> calificacionResponse = restTemplate.exchange(
                    getCalificacionUrl,
                    HttpMethod.GET,
                    requestEntity,
                    responseType
            ...
            if (calificacionResponse.getStatusCode() == HttpStatus.OK && calificacionResponse.getBody() != null) {
                List<Calificacion> calificaciones = calificacionResponse.getBody();
                logger.info("Se encontraron {} calificaciones para EstudianteCurso ID {}", calificaciones.size(), ec.getId());

                String comentarios = "";

                // Procesar cada calificación y asignarla al campo correspondiente
                for (Calificacion c : calificaciones) {


                    // Asignar retroalimentación (se sobrescribirá si hay múltiples, pero generalmente hay una)
                    if (c.getRetroalimentacion() != null && !c.getRetroalimentacion().trim().isEmpty()) {
                        ec.setRetroalimentacion(c.getRetroalimentacion());
                    }

                    // Asignar notas según el tipo
                    switch (c.getTipo()) {
                        case "P1":
                            ec.setP1(c.getNota());
                            logger.debug("P1 asignado: {} para EstudianteCurso ID {}", c.getNota(), ec.getId());
                            break;
                        case "P2":
                            ec.setP2(c.getNota());
                            logger.debug("P2 asignado: {} para EstudianteCurso ID {}", c.getNota(), ec.getId());
                            break;
                        case "P3":
                            ec.setP3(c.getNota());
                            logger.debug("P3 asignado: {} para EstudianteCurso ID {}", c.getNota(), ec.getId());
                            break;
                        case "EX":
                            ec.setEX(c.getNota());
                            logger.debug("EX asignado: {} para EstudianteCurso ID {}", c.getNota(), ec.getId());
                            break;
                        default:
                            logger.warn("Tipo de calificación desconocido: {} para EstudianteCurso ID {}", c.getTipo(), ec.getId());
                            break;
                    }

                    comentarios += ec.getRetroalimentacion() + "/n";

                }

               ec.setRetroalimentacion(comentarios);

            } else {
                logger.warn("No se pudieron obtener las calificaciones para EstudianteCurso ID {} o la respuesta no fue OK: {}",
                        ec.getId(), calificacionResponse.getStatusCode());
            }
        } catch (HttpClientErrorException ex) {
            logger.error("Error HTTP al obtener calificaciones para EstudianteCurso ID {}: {} - {}",
                    ec.getId(), ex.getStatusCode(), ex.getResponseBodyAsString());
        } catch (Exception ex) {
            logger.error("Error general al obtener calificaciones para EstudianteCurso ID {}: {}",
                    ec.getId(), ex.getMessage());
        }
    }

    /**
     * Metodo auxiliar para obtener la definitiva de un EstudianteCurso
     */
    private void obtenerDefinitiva(EstudianteCurso ec, HttpEntity<Void> requestEntity) {
        ...
        if(ec.getH() == null)
        {
            ...
            try {
                ...
                if (definitivaResponse.getStatusCode() == HttpStatus.OK && definitivaResponse.getBody() != null) {
                    Float definitiva = definitivaResponse.getBody();
                    ec.setDefinitiva(definitiva);
                    logger.info("Definitiva obtenida para EstudianteCurso ID {}: {}", ec.getId(), definitiva);
                } else {
                    logger.warn("No se pudo obtener la definitiva para EstudianteCurso ID {} o la respuesta no fue OK: {}. Se dejará como null.",
                            ec.getId(), definitivaResponse.getStatusCode());
                    ec.setDefinitiva(null);
                }
            } catch (HttpClientErrorException ex) {
                logger.error("Error HTTP al obtener definitiva para EstudianteCurso ID {}: {} - {}. Se dejará como null.",
                        ec.getId(), ex.getStatusCode(), ex.getResponseBodyAsString());
                ec.setDefinitiva(null);
            } catch (Exception ex) {
                logger.error("Error general al obtener definitiva para EstudianteCurso ID {}: {}. Se dejará como null.",
                        ec.getId(), ex.getMessage());
                ec.setDefinitiva(null);
            }
